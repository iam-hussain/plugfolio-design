...
**On-Gradient-Deepens.** Anything laid on a role gradient — a chip, a field, a well — is filled by **deepening** that hue, never by tinting it white. `--on-grad-fill: rgb(0 0 0 / .18)`. A translucent-white fill lifts the gradient towards the white type sitting on it: at 16% white the solid label measured **3.77–4.26** across the three roles and the placeholder **3.55–4.02**, all under the floor, in the one place type has nowhere else to go. Deepening puts every role at **6.75–7.59**, and `--on-grad-fill-hi` (`.28`) takes hover to **8.08–8.96** — hover must make a label *easier* to read, not harder, which the old white-tint hover got backwards at **3.28–3.69**. This is the type rule above applied one layer down — the ban on alpha-reduced type is worth nothing if the surface underneath it is alpha-lightened instead. Borders are exempt: they are not text.
...
### Named Rules
**Danger-Is-Not-Coral.** Destructive actions use `--danger` (`#CC3626`), `--danger-hover` (`#B3271A`) and `--danger-ink`, never `--coral`. Coral is a pale *tile* fill: white on it measures **2.30**, so a Delete button wearing it is unreadable however correct the hue feels. The measured set is white on `--danger` **5.09**, on hover **6.52**, and `--danger-ink` at **5.09** against a light surface. Dark mode swaps the ink to the pale coral (**6.95**), because a deep red on a dark surface falls to **2.45** — a destructive colour has to be re-measured per theme, not inherited.

**Scrim-Is-Measured.** Type over media the product does not control — a video poster, a creator's cover — is only legible if the scrim under it is measured against the *worst* frame, which is pure white. `--scrim` is `rgb(0 0 0 / .55)`, the point at which white type clears AA (4.74) against white underneath. The first pass used `.34`, which looks right on a mid-toned still and measures **2.38** on a bright one. A scrim chosen by eye is chosen against one frame; the frame is the variable.

**The Nested-Radius Rule.** A control inside a padded track takes the track's radius less the inset — `--r-nest`, currently `--r-image` less the 3px segmented-control inset. Reusing the parent radius on a child leaves two curves fighting inside 3px of each other, which reads as a rendering fault rather than a shape.
